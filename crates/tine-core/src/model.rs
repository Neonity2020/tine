...
impl Graph {
    ...
    fn direct_projection_progress(&self) -> Option<crate::direct_projection::ProjectionProgress> {
        let generation = self.cache_gen.load(std::sync::atomic::Ordering::Acquire);
        let projection = self
            .direct_projection
            .lock()
    ...
    fn direct_projection_page_aliases_with_owners(&self) -> Option<Vec<(String, String, String)>> {
        let generation = self.cache_gen.load(std::sync::atomic::Ordering::Acquire);
        let projection = self
            .direct_projection
            .lock()
    ...
    fn direct_projection_real_page_names(&self) -> Option<crate::query::RealPageNames> {
        let generation = self.cache_gen.load(std::sync::atomic::Ordering::Acquire);
        let projection = self
            .direct_projection
            .lock()
    ...
    fn direct_projection_reference_candidate_pages(
        &self,
        names_norm: &[String],
        kind: ReferenceKind,
    ) -> Option<(
    ...
    fn direct_projection_block_ref_counts(
        &self,
