...
struct PageAuthority {
    ...
    block_homes: Vec<DocumentId>,
    projected: Vec<u8>,
}

impl PageAuthority {
    /// The accepted address of one imported block: its id plus the home
    /// document the import path actually bound it to. Fixtures must not guess
    /// that home; every block is born in its own document.
    fn block(&self, index: usize) -> BlockLocation {
        BlockLocation {
            block_id: self.block_ids[index],
            home_document_id: self.block_homes[index],
        }
    }
}
...
impl AuthorityFixture {
    fn new(label: &str, pages: Vec<PageSpec>) -> Self {
        ...
        for (path, page_id, home_document_id, block_ids, block_homes) in authority {
...
fn concurrent_same_page_fallback_never_publishes_the_stale_local_draft_as_current() {
    ...
    let remote_outcome = engine.stage_ready(ValidatedBatch::new(remote));
    assert!(
        matches!(
            remote_outcome.disposition(),
    ...
    for unrelated_first in [false, true] {
        ...
        let edit = |page: usize, content: &str| {
            OperationTransaction::new(vec![SemanticOperation::EditBlockContent {
                block: fixture.pages[page].block(0),
                content: content.into(),
            }])
            .unwrap()
        };
