...
const RUST_PRINT_SITE_COUNT: usize = 20;
const ALLOWLIST: &[AllowedSite] = &[
    AllowedSite { file: "crates/tine-core/src/concord_ledger.rs", function: "run", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "content-free-error", why: "best-effort ledger update failure carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", function: "projection_diag", macro_name: "core_diag", occurrences: &[0], bucket: "a", class: "content-free-debug", why: "the projection lifecycle channel interpolates only counts, byte totals, durations, generations and booleans; the one error value reaching it is a ProjectionRefusal that is NOT a reportable failure, i.e. AwaitingFullInventory, whose Display is a fixed literal, and projection_lifecycle_lines_carry_no_graph_content holds every caller to that shape", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", function: "projection_worker", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "content-free-error", why: "projection directory creation carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection/integrity.rs", function: "finish", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "fixed-family-report", why: "the background integrity check reports found damage as a fixed sentence and no value", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection_lease.rs", function: "take_writer_lease", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "content-free-error", why: "projection lease acquisition carries only a std::io::Error, whose Display never includes the path; printed once per wait, not per retry", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", function: "report_projection_failure", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "fixed-family-report", why: "report_projection_failure's always-on line is the fixed failure family and no error value", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", function: "report_projection_failure", macro_name: "core_diag", occurrences: &[1], bucket: "b", class: "directed-core-detail", why: "report_projection_failure's detail line repeats the family with the raw error, for a directed investigation", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/model/page_parse.rs", function: "isolate_page_parse", macro_name: "core_diag", occurrences: &[0], bucket: "b", class: "directed-core-detail", why: "an isolated parse panic names the page file and the panic message, for a directed investigation (GH #594)", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/model/page_cache.rs", function: "parse_page_entries_with_permit", macro_name: "core_diag", occurrences: &[0], bucket: "d", class: "numeric-shape", why: "isolated search worker panic reports only a worker number", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/backend_error.rs", function: "diagnostic_line", macro_name: "eprintln", occurrences: &[0], bucket: "d", class: "forwarding-writer", why: "the one core stderr writer: it prints what a core_diag! site built, and every such site is classified in its own row", gate: "the caller's row" },
    AllowedSite { file: "crates/tine-core/src/model/save_path.rs", function: "force_save_page_at_revision", macro_name: "core_diag", occurrences: &[0], bucket: "a", class: "fixed-debug", why: "Guide-page refusal messages are fixed literals", gate: "cfg(debug_assertions)" },
    AllowedSite { file: "crates/tine-core/src/model/save_path.rs", function: "save_page", macro_name: "core_diag", occurrences: &[0], bucket: "a", class: "fixed-debug", why: "Guide-page refusal messages are fixed literals", gate: "cfg(debug_assertions)" },
    AllowedSite { file: "crates/tine-core/src/model/sync_file.rs", function: "sync_file", macro_name: "core_diag", occurrences: &[0], bucket: "b", class: "directed-core-detail", why: "a deferred reconcile repeats the raw I/O error, for a directed investigation (GH #594)", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/publish.rs", function: "publish_graph_documents_inner", macro_name: "core_diag", occurrences: &[0, 1], bucket: "a", class: "content-free-debug", why: "publication refusals report only a fixed shape or collision count", gate: "runtime_debug_diagnostics_enabled" },
...
fn print_sites() -> Vec<(PrintSite, usize)> {
    ...
    let print_macro = Regex::new(r"\b(eprintln|println|dbg|core_diag)!\s*[({\[]").unwrap();
