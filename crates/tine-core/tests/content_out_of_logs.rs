...
const ALLOWLIST: &[AllowedSite] = &[
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[1689], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "checkpoint writer thread spawn carries only a std::io::Error", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[1736], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "publish_capture's error is a bounded literal or an object-store error over digest-named private blobs", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[9770], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "validate_and_apply phase timing contains a phase index and a duration", gate: "TINE_PHASE_TRACE" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[12087, 15816, 16843, 16871, 17050, 19791, 19820, 19854, 19886, 20751, 20777, 20924, 20943, 23901], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "engine diagnostics run only under explicit performance, CRDT, or activation trace flags", gate: "TINE_PHASE_TRACE/TINE_CRDT_TRACE/TINE_ACTIVATION_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8146, 8150, 8176, 8188], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "disposable checkpoint fallbacks carry typed store/decode errors describing batch ids, sealed kinds and sequences", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8352, 22178, 22416, 23281, 23325, 23357], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "receipt, pending-projection, and conflict-resolution reports contain only counts or fixed states", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[12884], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "CRDT writer-lane qualification reports a fixed disposition, an incarnation number and id, and a causal counter", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[21858, 21949, 22015], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "foreground mutation detail is available only for explicitly enabled runtime debugging", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[22809], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "actor tick report contains a fixed branch label, duration, and pending count", gate: "TINE_TICK_TRACE" },
