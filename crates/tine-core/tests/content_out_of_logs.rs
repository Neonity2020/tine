...
const ALLOWLIST: &[AllowedSite] = &[
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[2359], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "checkpoint writer thread spawn carries only a std::io::Error", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[2482], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "orphan-image cleanup errors are bounded literals or capability errors over digest-named private checkpoint objects", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[2492], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "publish_capture's error is a bounded literal or an object-store error over digest-named private blobs", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[10467], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "validate_and_apply phase timing contains a phase index and a duration", gate: "TINE_PHASE_TRACE" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[12784, 16513, 17540, 17568, 17747, 20488, 20517, 20551, 20583, 21455, 21481, 21628, 21647, 24672], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "engine diagnostics run only under explicit performance, CRDT, or activation trace flags", gate: "TINE_PHASE_TRACE/TINE_CRDT_TRACE/TINE_ACTIVATION_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/object_store.rs", lines: &[2289], macro_name: "eprintln", bucket: "b", class: "enum-trace", why: "immutable publication reports only a fixed artifact class", gate: "TINE_PUBLISH_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[7289], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "watcher trace is explicitly enabled for a directed investigation", gate: "TINE_CLEAN_WATCHER_TRACE" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8191, 8209], macro_name: "eprintln", bucket: "a", class: "numeric-debug", why: "clean-open stage and counter reports contain fixed names and numeric measurements", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8386, 8390, 8417, 8429], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "disposable checkpoint fallbacks carry typed store/decode errors describing batch ids, sealed kinds and sequences", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8596, 22599, 22837, 24369, 24413, 24445], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "receipt, pending-projection, and conflict-resolution reports contain only counts or fixed states", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[13263], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "CRDT writer-lane qualification reports a fixed disposition, an incarnation number and id, and a causal counter", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[22279, 22370, 22436], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "foreground mutation detail is available only for explicitly enabled runtime debugging", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[23233], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "actor tick report contains a fixed branch label, duration, and pending count", gate: "TINE_TICK_TRACE" },
