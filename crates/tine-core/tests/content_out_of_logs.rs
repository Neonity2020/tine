...
const ALLOWLIST: &[AllowedSite] = &[
    ...
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[2080], macro_name: "eprintln", bucket: "d", class: "fixed-family-report", why: "report_projection_failure's always-on line is the fixed failure family and no error value", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[2082], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "report_projection_failure's detail line repeats the family with the raw error, for a directed investigation", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[2150], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "projection directory creation carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[2168], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "projection lease acquisition carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[2460], macro_name: "eprintln", bucket: "a", class: "fixed-debug", why: "the deferred-turn line is the ELSE arm of is_reportable_failure, so the only ProjectionRefusal that reaches it is AwaitingFullInventory, whose Display is a fixed literal; Failed(_) takes the reviewed report_projection_failure path instead", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[15717], macro_name: "eprintln", bucket: "d", class: "numeric-shape", why: "isolated search worker panic reports only a worker number", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[21695, 24663], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "reconcile and isolated-parse failures contain no path, title, content, or raw error", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[23454, 23643], macro_name: "eprintln", bucket: "a", class: "fixed-debug", why: "Guide-page refusal messages are fixed literals", gate: "cfg(debug_assertions)" },
    AllowedSite { file: "crates/tine-core/src/oplog/batch.rs", lines: &[213], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "batch composition contains public enum kinds and numeric sizes only", gate: "TINE_BATCH_TRACE" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[3209], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "checkpoint writer thread spawn carries only a std::io::Error", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[3223], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "orphan-image cleanup errors are bounded literals or capability errors over digest-named private checkpoint objects", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/checkpoint_generation.rs", lines: &[3247], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "publish_capture's error is a bounded literal or an object-store error over digest-named private blobs", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[10741], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "validate_and_apply phase timing contains a phase index and a duration", gate: "TINE_PHASE_TRACE" },
    AllowedSite { file: "crates/tine-core/src/oplog/hot_engine.rs", lines: &[13064, 16823, 17850, 17878, 18057, 20798, 20827, 20861, 20893, 21765, 21791, 21938, 21957, 24982], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "engine diagnostics run only under explicit performance, CRDT, or activation trace flags", gate: "TINE_PHASE_TRACE/TINE_CRDT_TRACE/TINE_ACTIVATION_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/object_store.rs", lines: &[2313], macro_name: "eprintln", bucket: "b", class: "enum-trace", why: "immutable publication reports only a fixed artifact class", gate: "TINE_PUBLISH_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/semantic.rs", lines: &[1006], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "semantic snapshot diagnostic contains counts and encoded byte sizes", gate: "TINE_SEMANTIC_TRACE" },
    AllowedSite { file: "crates/tine-core/src/oplog/sqlite.rs", lines: &[1995, 4742, 4746, 4753, 4766, 4778, 4786, 4818, 5611], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "SQLite construction diagnostics run only under explicit trace flags", gate: "TINE_PHASE_TRACE/TINE_ACTIVATION_TRACE" },
    ...
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[7521], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "watcher trace is explicitly enabled for a directed investigation", gate: "TINE_CLEAN_WATCHER_TRACE" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8423, 8441], macro_name: "eprintln", bucket: "a", class: "numeric-debug", why: "clean-open stage and counter reports contain fixed names and numeric measurements", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8618, 8622, 8649, 8661], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "disposable checkpoint fallbacks carry typed store/decode errors describing batch ids, sealed kinds and sequences", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[8836, 22884, 23122, 24958, 25002, 25034], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "receipt, pending-projection, and conflict-resolution reports contain only counts or fixed states", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[13516], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "CRDT writer-lane qualification reports a fixed disposition, an incarnation number and id, and a causal counter", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[22564, 22655, 22721], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "foreground mutation detail is available only for explicitly enabled runtime debugging", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[23518], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "actor tick report contains a fixed branch label, duration, and pending count", gate: "TINE_TICK_TRACE" },
