...
| `Graph::export_query_subtrees` / `application_captured_export` | `crates/tine-core/src/query/export_execute.rs` | which subtrees a query exports, bounded | cut | `PreparedExportBatch::execute` | 2 | traversal adapters and actor merged registry | one SQL executor and subtree hydrator; old walks are test-only | not separately timed | `c7b_query_driver_parity`, `public_export_reads_committed_subtrees_without_source_documents` |
...
| `journal_ordinal` | `crates/tine-core/src/query.rs` | which day a journal page evaluates as | cut for the query adapters | `JournalFormat::parse` via `ApplicationQueryPage::journal` | 2 → 1 | 2 producers, disagreeing on any custom `:journal/page-title-format` | 1 config-aware producer for both backends | none | The sparse-path exception this row recorded is gone: the RET2 correction deleted the sparse task-candidate evaluator and its `ApplicationSparseQueryPage`. The one surviving `journal_ordinal` caller is `resolve_date_token`, which reads a `(between …)` bound literal the user typed rather than a page's day, in code shared by both backends, so it cannot diverge. `REG-W4-C7B-MANAGED-JOURNAL-ORDINAL-001` |
