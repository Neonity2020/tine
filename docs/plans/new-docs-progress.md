...
- **Discrepancy 1 — stale "Open all results" label.** `docs/FEATURES.md:443` ("**Open all results** creates the persistent workspace") and `crates/tine-core/src/templates/tips.md:8` ("Choose **Open all results**") name a control that does not exist in the shipped UI; the footer button reads "**Open search tab**" (`src/components/QuickSwitcher.tsx:597`, no "Open all results" anywhere under `src/`). The new pages use the shipped label. tips.md/FEATURES.md wording was NOT corrected: FEATURES.md is outside this dossier's edit set and tips.md changes beyond the added pointer link were not authorized — flagged for the manager as a one-label fix.
- **Discrepancy 2 — stale save phrasing in tips.md:9.** "Rename it when the search is worth keeping: Tine turns it into an ordinary query page" predates the shipped form; the workspace shows the "Name this search to save it as a page" input + "**Save page**" button. New pages use the shipped labels; tips.md left as-is (same authorization boundary).
- **Discrepancy 3 — "Search settings" vs shipped location.** tips.md:10 says ranking history can be disabled/reset "under Search settings"; the shipped control is Settings → **Editor** → Advanced → "**Learn Ctrl+K choices**" (`src/components/Settings.tsx:1719-1726`). New pages name the shipped path.
