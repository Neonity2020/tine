...
| `DIRECT-REF-CREATE-UNREADABLE-OWNER` | Sync delivery, an interrupted external write, or malformed imported Markdown/Org leaves a page file Tine cannot read or parse, so its effective name is unknown; creating a page of that name would give one name two files | Refuse name-only creation only for a name that failed file could be: its file-name name, or a name its bytes, folded as page names are, contain (`failures_that_could_own`). A failure naming no single file, or a file unreadable now, could be any page and refuses every name-only creation. Name the file in the error. Every other creation, save and the watcher proceed: a content rejection is reconciled state recorded in `page_index_failures` and announced once (`graph-unreadable-pages`), not a cycle failure to retry |
